...
class SpambotUserspamForm extends FormBase {
   ...
   *   The extension list service for modules.
   */
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?UserInterface $user = NULL) {
    ...
    if ($key) {
      ...
      $form['action']['report']['#description'] = $this->t('An API key from <a href="https://www.stopforumspam.com">www.stopforumspam.com</a> must <a href="@admin-url">be configured</a> to report spammers.', ['@admin-url' => $url]);
    }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($key_required && !$this->config->get('spambot_sfs_api_key')) {
      $form_state->setErrorByName('action',
        $this->t('To report spammers to www.stopforumspam.com, you need to register for an API key at <a href="https://www.stopforumspam.com">www.stopforumspam.com</a> and enter it into the @page.', [
          ...
          if (!empty($data['ip']['appears'])) {
            $messages[] = [
              'text' => $this->t('An IP address @ip used by this account matches %num times.', [
                '@ip' => Link::fromTextAndUrl($ip, Url::fromUri('https://www.stopforumspam.com/search?q=' . $ip))->toString(),
  ...
  public function sfsRequestDataMessage($request, $data, $field) {
    return [
      'text' => $this->t('This account\'s @field address matches %num times: <a href=":href" target="_blank">@field_req</a>.', [
        '@field' => $field,
        ':href' => "https://www.stopforumspam.com/search?q={$request[$field]}",
        '@field_req' => $request[$field],
  ...
  public function defaultBatchBuilderSettings() {
    ...
    $this->batchBuilder->setFile($this->moduleList->getPath('spambot') . '/src/Form/SpambotUserspamForm.php');
  }
